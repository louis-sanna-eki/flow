#!/usr/bin/perl
#jompo BEGIN {push @INC, '/var/www/cgi-bin/flow/'}
use lib '/var/www/cgi-bin/flow/';
use strict;
use warnings;
...
my ($db, $card, $id, $lang, $alph, $from, $to, $rank, $search, $schstr, $searchtable, $searchid, $mode, $privacy, $limit, $test);

$db = url_param('db') || 'aleurodes'; 
$card = url_param('card');
# jompo added if defined
if ( ! defined($card) ) {
	$card='';
}
$id = url_param('id');
...
# sofiz my $search = url_param('search');
$search = url_param('search');
for (my $i = 1; $i < scalar(keys(%types)) + 1; $i++) {
#sofiz	my $schstr = param("search$i");
	$schstr = param("search$i");
	# jompo added if defined
	if( defined $schstr ) {
...
#	system 		"/var/www/html/perl/explorer20.pl $argvtop -mode=" . url_param('card');
	my $xxx = url_param('card');
	if ( defined $xxx ) {
		system 		"/var/www/html/perl/explorer20.pl $argvtop -mode=" . $xxx;
	}
	print		"</TABLE>";
	print 	"</TD></TR>";
	if (!$card) {
...
#sofiz  sans apo ->marchepas	if ($_->[1] =~ m|[^A-Z a-z 0-9 : , ( ) \[ \] ! _ = & ° . * ; “ " ” \\ \- – ? ‡ \n ]|) {
	#			my ($res) = @{request_row("SELECT reencodage('".$_->[1]."');", $dbh)};
				#jompo my ($res) = @{request_row(E'"SELECT $_->[1];"', $dbh)};
