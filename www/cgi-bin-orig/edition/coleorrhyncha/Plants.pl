...
sub HPinsert {
	...
	while ($i<$nb) {
		...
		if ($rank eq 'family') {
			...
			my ($c) = @{request_tab("SELECT count(*) FROM plantes WHERE nom = '$name' AND ref_parent = $gen;", $dbc, 1)};
	...
	print 	html_header(\%headerHash),
		...
		div({-class=>"wcenter"},
			...
			a({-href=>'Plants.pl?action=get', -style=>'color: blue; text-decoration: none;'}, "Modify host plants"), br, br, br, br,
...
sub HPform {
	...
	unless ($nb) {
		...
		$back = span(	{-onMouseover=>"document.backbtn.src=eval('backonimg.src')",
				-onMouseout=>"document.backbtn.src=eval('backoffimg.src')",
				-onClick=>"PlantsForm.action = 'Plants.pl?action=get'; PlantsForm.submit();"
				},
				img({-border=>0, -src=>'/Editor/back0.png', -name=>"backbtn"})
			);
