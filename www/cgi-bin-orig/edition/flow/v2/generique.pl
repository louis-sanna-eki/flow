...
if ($table eq 'pays' or $table eq 'statuts' or $table eq 'langages' or $table eq 'periodes' or $table eq 'niveaux_geologiques' or $table eq 'lieux_depot' or $table eq 'sexes') { foreach (@files) { push(@confs, get_connection_params($_)) } }
