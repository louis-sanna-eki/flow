...
sub getThesaurusItems {
	...
	if ($table eq 'taxons') { 
		...
		$req = "SELECT t.index || '#' || nc.index,
			...
			ELSE
				nc.orthographe || coalesce(' ' || nc.autorite, '') || coalesce(' (' || (SELECT orthographe FROM noms WHERE index = n.ref_nom_parent) || ')', '') 
				|| coalesce(' = ' || nt.orthographe, '') || coalesce(' ' || nt.autorite, '') || coalesce(' (' || (SELECT orthographe FROM noms WHERE index = n2.ref_nom_parent) || ')', '')
			END,
			nc.orthographe,
			nc.autorite,
			txn.ref_statut
	...
	elsif ($table eq 'plants') { 

		if ($id) { $where = "WHERE get_host_plant_name(index) ILIKE '%$expr%' ORDER BY fullname"; }
		else { $where = "WHERE index = $expr"; }

		$req = "SELECT index, get_host_plant_name(index) AS fullname FROM plantes $where;";
		...
		if ($id) { $where = "WHERE reencodage(l.nom) ILIKE reencodage('%$expr%') ORDER BY l.nom, r.nom, p.en"; }
		else { $where = "WHERE l.index = $expr"; }

		$req = " SELECT l.index, l.nom || coalesce(', ' || r.nom,'') || coalesce(' (' || p.en || ')','') 
			 FROM localites AS l 
...
sub Xform {
	...
	#join(br, map { "$_ = ".url_param($_) } url_param()). br.
	"</DIV>".
	"</BODY>".
	"</HTML>";
