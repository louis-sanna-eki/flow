...
sub treat_data {
	...
	unless ($nameID or ($nameID = testpreexistence() and $action0 eq 'insert')) {
		...
		if ($action0 eq 'insert' and !$nameID) {
			...
			while (!$end) {
				...
				$prvreq = "SELECT ref_nom FROM taxons_x_noms WHERE ref_nom_cible IN (".join(', ', @alter).") AND ref_statut IN (4, 12, 15, 25) AND ref_nom NOT IN (".join(', ', @alter).");";
				$nxtreq = "SELECT ref_nom_cible FROM taxons_x_noms WHERE ref_nom IN (".join(', ', @alter).") AND ref_statut IN (4, 12, 15, 25) AND ref_nom_cible NOT IN (".join(', ', @alter).");";
	...
	unless ($msg) {
		...
		if ($OID and $action0 eq 'insert') {
			...
			#}
			#$req = "UPDATE taxons_x_noms SET ".join(', ' , map("$_ = ?", @fields)).", date_modification = ('now'::text)::date WHERE oid = $OID;";
			#$sth = $dbc->prepare($req) or die "$req \n ";
			...
			if ($action0 eq 'insert') {
				$req = "INSERT INTO taxons_x_noms (".join(', ', @fields).", date_creation, date_modification) VALUES (".join(', ', ('?') x scalar(@fields)).", ('now'::text)::date, ('now'::text)::date);";
				...
				# Case of new identification => insert misidentification
					...
					if ($taxon2ID) {
						...
						$req = "INSERT INTO taxons_x_noms 
							...
							VALUES (".join(', ', ('?') x scalar(@tab)).",('now'::text)::date,('now'::text)::date);";
						$sth = $dbc->prepare($req) or die "$req ".$dbc->errstr;
				...
				# Case of homonymy => make reciprocal link
					...
					unless ($testh) {
						...
						$req = "INSERT INTO taxons_x_noms (ref_taxon, ref_nom, ref_nom_cible, ref_statut, nom_label, nom_cible_label, ref_publication_denonciation, page_denonciation, date_creation, date_modification) 
							VALUES (?,?,?,?,?,?,?,?,('now'::text)::date,('now'::text)::date);";
						$sth = $dbc->prepare($req) or die "$req ".$dbc->errstr;
						$sth->execute(@tab) or die "$req ".$dbc->errstr;	
					}
				}
				...
				if ($OID and $OID != $OID0) { $msg = 'The data is already in the database'; $case = 'fail'; }
					...
					$req = "UPDATE taxons_x_noms SET ".join(', ' , map("$_ = ?", @fields)).", date_modification = ('now'::text)::date WHERE oid = $OID0;";
					...
					if ($statusID0 == 21 and $statusID == 21 and $taxonID0 == $taxonID and $nameID0 == $nameID) {
						...
						$req = "UPDATE taxons_x_noms SET ref_publication_denonciation = ?, page_denonciation = ?, date_modification = ('now'::text)::date WHERE ref_taxon != $taxonID AND ref_nom = $nameID AND ref_statut = 21;";
						$sth = $dbc->prepare($req) or die "$req \n @v";
					...
					if (($taxonID0 != $taxonID or $statusID0 != $statusID) and $nameID0 == $nameID) {
						...
						if ($statusID0 == 1 && $statusID != 1 && $statusID != 20) {
							...
							$req = "UPDATE taxons_x_noms SET ref_taxon = $taxonID, date_modification = ('now'::text)::date WHERE ref_taxon = $taxonID0;";
							$sth = $dbc->prepare($req) or die "$req ".$dbc->errstr;
						...
						# change taxon and keep taxonomic status
							...
							while (scalar(@prev)) {
								...
								@prev = ();
								foreach (@{$res}) {
									$req = "UPDATE taxons_x_noms SET ref_taxon = $taxonID, date_modification = ('now'::text)::date WHERE oid = ".$_->[0].";";
									$sth = $dbc->prepare($req) or die "$req ".$dbc->errstr;
									$sth->execute() or die "$req ".$dbc->errstr;
...
sub is_taxonomic_status {
	my ($statut) = @_;
	my @unvalids = (3,5,8,10,17,18,20,21,22);
	my $result = 1;
	...
	if ($table eq 'taxons') { 
		...
		if ($id eq 'sciname') {
			$xfields = "NULL, 
				...
				r.ordre,
				CASE WHEN (SELECT ordre FROM rangs WHERE index = n.ref_rang) > (SELECT ordre FROM rangs WHERE en ILIKE 'genus') THEN
					nc.orthographe || coalesce(' ' || nc.autorite, '') || coalesce(' [' || txn.ref_taxon || ']', '')
				ELSE
					nc.orthographe || coalesce(' ' || nc.autorite, '') || coalesce(' (' || (select all_to_rank(nc.index, 'subtribe', 'family', ',', 'down', 'notfull')) || ')', '') || coalesce(' [' || txn.ref_taxon || ']', '')
				END";
			$xfields2 = "NULL, 
				txn.ref_nom_cible, 
				r.ordre,
				CASE WHEN (SELECT ordre FROM rangs WHERE index = n.ref_rang) > (SELECT ordre FROM rangs WHERE en ILIKE 'genus') THEN
					nt.orthographe || coalesce(' ' || nt.autorite, '') || coalesce(' [' || txn.ref_taxon || ']', '')
				ELSE
					nt.orthographe || coalesce(' ' || nt.autorite, '') || coalesce(' (' || (select all_to_rank(nt.index, 'subtribe', 'family', ',', 'down', 'notfull')) || ')', '') || coalesce(' [' || txn.ref_taxon || ']', '')
				END";
		} else {
			$xfields = "txn.ref_taxon, 
				...
				r.ordre,
				CASE WHEN (SELECT ordre FROM rangs WHERE index = n.ref_rang) > (SELECT ordre FROM rangs WHERE en ILIKE 'genus') THEN
					nc.orthographe || coalesce(' ' || nc.autorite, '') || coalesce(' = ' || nt.orthographe, '') || coalesce(' ' || nt.autorite, '') || coalesce(' [' || txn.ref_taxon || ']', '')
				ELSE
					nc.orthographe || coalesce(' ' || nc.autorite, '') || coalesce(' (' || (select all_to_rank(nc.index, 'subtribe', 'family', ',', 'down', 'notfull')) || ')', '') 
					|| coalesce(' = ' || nt.orthographe, '') || coalesce(' ' || nt.autorite, '') || coalesce(' (' || (select all_to_rank(nt.index, 'subtribe', 'family', ',', 'down', 'notfull')) || ')', '')
			...
			$xfields2 = "txn.ref_taxon, 
				...
				r.ordre,
				CASE WHEN (SELECT ordre FROM rangs WHERE index = n.ref_rang) > (SELECT ordre FROM rangs WHERE en ILIKE 'genus') THEN
					nt.orthographe || coalesce(' ' || nt.autorite, '') || coalesce(' = ' || nc.orthographe, '') || coalesce(' ' || nc.autorite, '') || coalesce(' [' || txn.ref_taxon || ']', '')
				ELSE
					nt.orthographe || coalesce(' ' || nt.autorite, '') || coalesce(' (' || (select all_to_rank(nt.index, 'subtribe', 'family', ',', 'down', 'notfull')) || ')', '') 
					|| coalesce(' = ' || nc.orthographe, '') || coalesce(' ' || nc.autorite, '') || coalesce(' (' || (select all_to_rank(nc.index, 'subtribe', 'family', ',', 'down', 'notfull')) || ')', '')
					|| coalesce(' [' || txn.ref_taxon || ']', '')
				END";
		} 
		...
		foreach ( sort {$a->[4] cmp $b->[4] || $a->[5] cmp $b->[5] || $a->[6] cmp $b->[6] || $a->[3] cmp $b->[3] } @{$res}) {
			if(lc($_->[4]) eq lc($expr)) { $full .= $table.'["'.$_->[3].$_->[7].'"] = "'.$_->[0].'_COL_'.$_->[1].'_COL_'.$_->[4].'_COL_'.$_->[2].'"; '; }
			else { $partial .= $table.'["'.$_->[3].$_->[7].'"] = "'.$_->[0].'_COL_'.$_->[1].'_COL_'.$_->[4].'_COL_'.$_->[2].'"; '; }
		}
		...
		$req = 	"SELECT txn.oid,
				CASE WHEN nc.index != nt.index THEN
					CASE WHEN (SELECT ordre FROM rangs WHERE index = n.ref_rang) > (SELECT ordre FROM rangs WHERE en ILIKE 'genus') THEN
						nc.orthographe || coalesce(' ' || nc.autorite, '') || 
