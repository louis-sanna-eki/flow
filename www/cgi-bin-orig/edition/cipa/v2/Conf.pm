...
my $dbh = db_connection($config, 'EXPLORER');
our $dblabel = $config->{'DB_NAME'} || 'Flow';

# submit query in sql (return an array ref)
...
our $js = "
	...
	statdef[14]	=	'Set a name as forsaken';
...
sub get_single_table_thesaurus {
	...
	if ($table eq 'editions') { 
		...
		make_thesaurus("SELECT index, en, tdwg_level FROM pays WHERE index in (SELECT index FROM pays AS p WHERE (SELECT reencodage(en) FROM pays AS p2 WHERE p2.parent = p.tdwg LIMIT 1) NOT LIKE '%'||reencodage(p.en)||'%' OR tdwg_level = '4') ORDER BY en;", 'tdwg', '$intitule = $row->[1];', $retro, $onload, $dbc);
		...
		make_thesaurus("SELECT index, en, tdwg, tdwg_level FROM pays WHERE (tdwg_level in ('1', '2', '4') OR index in (SELECT index FROM pays WHERE tdwg_level = '3' and en not in (SELECT DISTINCT en FROM pays WHERE tdwg_level = '4'))) ORDER BY en;", 'tdwg', '$intitule = "$row->[1] [$row->[3]]";', $retro, $onload, $dbc);
	}
	elsif ($table eq 'regions') { 
		make_thesaurus("SELECT index, en, tdwg, tdwg_level FROM pays WHERE (tdwg_level in ('1', '2', '4') OR index in (SELECT index FROM pays WHERE tdwg_level = '3' and en not in (SELECT DISTINCT en FROM pays WHERE tdwg_level = '4'))) ORDER BY en;", 'tdwg', '$intitule = "$row->[1] [$row->[3]]";', $retro, $onload, $dbc);
	}
...
sub makeAuthorsfields {
	...
	my $fields = 	"<div style='position: relative; margin: 0; padding: 0; height: 22px;' >
				...
				>
					<img src='/dbtntDocs/next.png' border='0' onMouseOver=\"this.style.cursor = 'pointer';\">
				</div>				
				...
				>
					...
					<img src='/dbtntDocs/ok.png' border='0' onMouseOver=\"this.style.cursor = 'pointer';\">
				</div>
			</div>";
}
