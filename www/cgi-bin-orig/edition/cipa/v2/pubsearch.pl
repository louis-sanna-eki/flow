...
sub pub_search_options {
	...
	my $pubAOpts = table({-border=>0 , -cellspacing=>10},
			Tr(
				td(
				"<SELECT NAME='searchAuthor' onChange='testspc(this.form.searchAuthor);' CLASS=PopupStyle STYLE='text-align: center; border: 1px solid #888888;'>$pubAuthorOptions"
				),
				td("*Optional")
			)	
		);
	...
	if ($from) {
		...
		$backbtn .= start_form(-name=>'backform', -method=>'post',-action=>'').
			...
			img({-onMouseOver=>"this.style.cursor = 'pointer';", -onClick=>"clearSearchParams(backform); backform.action = '$from'; backform.submit();", -border=>0, -src=>'/Editor/back.png', -name=>"backimg"}).
	...
	div({-class=>'wcenter'},
		...
		"<TABLE><TR><TD>",
		img({-onMouseOver=>"this.style.cursor = 'pointer';", -onClick=>"if(pubsearchform.searchYear.value) { pubsearchform.action = 'pubsearch.pl?action=getPub'; pubsearchform.submit(); } else { alert('You have to select a publication year.') }", -border=>0, -src=>'/Editor/ok.png', -name=>"ok"}),
		"</TD>",
		arg_persist(),
...
sub pub_search_form {
	...
	$html .= 	html_header(\%headerHash).
			...
			div({-class=>'wcenter'},
				...
				div({-id=>'fixe0', -style=>'background: transparent;'},
						...
						Tr(
							td(
								img({	-onMouseOver=>"this.style.cursor = 'pointer';",
									...
									-border=>0, -src=>'/Editor/back.png', -name=>"backimg"})
							),
							td( $newbtn )
						)
					),
