...
if ($table eq 'pays' or $table eq 'statuts' or $table eq 'langages' or $table eq 'periodes' or $table eq 'niveaux_geologiques' or $table eq 'lieux_depot' or $table eq 'types_association') { foreach (@files) { push(@confs, get_connection_params($_)) } }
if($table eq 'plantes') {
