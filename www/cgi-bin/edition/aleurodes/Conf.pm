...
our $conf_file = '/etc/flow/aleurodseditor.conf';

# Editor name
my $config = get_connection_params($conf_file);
my $dbh = db_connection($config, 'DEFAULT');
our $dblabel = $config->{'DB_NAME'} || 'Whiteflies';

our $imgs_url = 'http://hemiptera.infosyslab.fr/aleurodfotos/';
...
sub get_single_table_parameters { 
	...
	if ($table eq 'revues') { 
		...
		${$label} = "'<TABLE><TR><TD>' || x.url || '</TD><TD></TD></TR></TABLE>'";
