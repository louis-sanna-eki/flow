...
#---- jompo -----
#my (@jompo) = @{request_tab("select column_name from information_schema.columns where table_name = 'taxons_x_noms';", $dbc, 1)};
#my (@jompo) = @{request_tab("select txn.oid from taxons_x_noms AS txn;", $dbc, 1)};
#my (@jompo) = @{request_tab("select * from information_schema.tables;", $dbc, 1)};
#print STDERR "JOMPO\n";
#print STDERR "Size: ",scalar @jompo,"\n";
#print STDERR "\$jompo[0] = @jompo[0]\n";
#foreach (@jompo) {
#foreach ($_) {
#  print STDERR "$_\n";
#}
...
if ($action eq 'insert') {
	...
	#}
	#else {
		$req = "SELECT t.ref_taxon_parent, n.ref_nom_parent,n.cavernicolous, n.cavernicolous_type, r.ordre, n.orthographe, n.annee, n.parentheses, n.fossil, nc.orthographe || coalesce(' ' || nc.autorite, '')
			FROM taxons AS t
			LEFT JOIN noms AS n ON n.index = $nid
			LEFT JOIN noms_complets AS nc ON nc.index = n.ref_nom_parent
			LEFT JOIN taxons AS tp ON tp.index = t.ref_taxon_parent
...
sub get_data {
	...
	$req = "SELECT n.orthographe, n.annee, n.parentheses, n.fossil, n.gen_type, n.ref_type_designation, n.ref_publication_designation, n.page_designation, n.ref_publication_princeps, n.page_princeps, nc.orthographe || coalesce(' ' || nc.autorite, ''),  n.cavernicolous, n.cavernicolous_type FROM noms AS n LEFT JOIN noms_complets AS nc ON nc.index = n.index WHERE n.index = ".$result->[0][1].";";
	...
	if ($name->[0][11]) {
	    param('cavernicolous', $name->[0][11]);
	}

	if ($name->[0][12]) {
        param('cavernicolous_type', $name->[0][12]);
    }
...
sub treat_data {
	...
	my $cavernicolous	= param('cavernicolous');
	my $cavernicolous_type = param('cavernicolous_type');
	...
	unless ($nameID or ($nameID = testpreexistence() and $action0 eq 'insert')) {
		...
		#Cavernicolous
        if ($cavernicolous) {
            $fields_values{"cavernicolous"} = "true";
            $fields_insert{"cavernicolous"} = "?";
        } else {
            $fields_values{"cavernicolous"} = 'false';
            $fields_insert{"cavernicolous"} = "?";
        }
        if($cavernicolous_type) {
            $fields_values{"cavernicolous_type"} = $cavernicolous_type;
            $fields_insert{"cavernicolous_type"} = "?";
        } else {
            $fields_values{"cavernicolous_type"} = undef;
            $fields_insert{"cavernicolous_type"} = "?";
        }
...
sub get_name {	
	...
	my $order = param('nameOrder') ? "AND r.ordre = ".param('nameOrder') : '';
...
sub getThesaurusItems {
	...
	if ($table eq 'taxons') { 
		...
		# jompo, si on remplace txn.oid par txn.ref_nom, ca va plus loin mais ca plante, donc, il manque le champ oid ???
		$req = 	"SELECT txn.oid,
...
sub name_form {
		...
		-onKeyUp => "	if(event.keyCode == 9 || event.keyCode == 13 || event.keyCode == 27 || event.keyCode == 38 || event.keyCode == 40) {
					...
					function callServerScript() { 
						if(document.getElementById('$fieldID').value.length > 2) { 
							getThesaurusItems(['args__$fieldID', 'args__taxons', 'args__'+encodeURIComponent(document.getElementById('$fieldID').value), 'args__AND txn.ref_statut NOT IN (8,17,18,20,21,22)', 'NO_CACHE'], [setThesaurusItems]);
						} 
						else {  
	...
	## designation types
	my $tabcavernicolous;
    $tabcavernicolous = request_tab("SELECT index, en FROM types_cavernicolous ORDER BY en;", $dbc, 2);

    my $cavernicolousTypes;
    $cavernicolousTypes->{0} = '-- cavernicolous type --';
    foreach (@{$tabcavernicolous}) {
        $cavernicolousTypes->{$_->[0]} = lc($_->[1]);
    }
	...
	my $checker = param('cavernicolous') && param('cavernicolous')==1 ? "checked":"unchecked";

    $cdisp = param('cavernicolous') && param('cavernicolous') ==1 ? 'inline' : 'none';
    my $xcavernicolous =
    table({-cellspacing=>0, -cellpadding=>0, -style=>'display: inherit;'},
        #Tr(
        #    td( checkbox(-name=>'cavernicolous', -id=>'cavernicolous', -value=>1, -label=>'', -onFocus=>'AutoComplete_HideAll(); Reset_TabIndex();', -onClick=>'clearSearchedName();')."\n" )."\n",
        #    td( span({-style=>'color: navy; font-size: 13px;'}, 'cavernicolous')."\n" )."\n"
        #)."\n",
        Tr(
        					td( checkbox(
        						-name=>'cavernicolous',
        						-id=>'cavernicolous',
        						-value=>param('cavernicolous') || 0,
        						-label=>'',
        						-onLoad=>"
        						        var isCheck ='".$checker."';
        						        if(isCheck=='checked'){
        						...
        						-onClick=>"	clearSearchedName();
        								if (this.checked) {
        									document.getElementById('cavernicolous_type').style.display = 'inline';
        									document.getElementById('cavernicolous').value = 1;
        								} else {
        									document.getElementById('cavernicolous_type').style.display = 'none';
        									document.getElementById('cavernicolous_type').selectedIndex = 0;
        									document.getElementById('cavernicolous').value = 0;
        									document.getElementById('pubField1').style.display = 'none';
        									clearSearchedPub(1);
        									clearPub(1);
        									hidePub(1);
        					...
        					td( span({-style=>'color: navy; font-size: 13px; padding-right: 6px;'}, 'cavernicolous')."\n" )."\n",
        					td(
        						popup_menu(
        							-name=>"cavernicolous_type",
        							-id=>"cavernicolous_type",
        							-style=>"display: $cdisp;",
        							-value=>param('cavernicolous_type') || '-- cavernicolous type --',
        							-values=>[sort { $cavernicolousTypes->{$a} cmp $cavernicolousTypes->{$b} } keys(%{$cavernicolousTypes})],
        							-labels=>$cavernicolousTypes,
        							-onFocus=>'AutoComplete_HideAll(); Reset_TabIndex();',
        							-onChange=>""
        						)."\n"
        					)."\n"
