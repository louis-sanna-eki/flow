...
sub modify_token {
	...
	if ( my $dbc = db_connection($config) ) { # connection
		...
		my $pt = param("pt");
		my $de = param("de");
        #print("<script> alert(".reencodage($fr).")</script>");
		my $sth = $dbc->prepare( "UPDATE traductions SET id = ?, en = ?, fr = reencodage(?), es = ?, pt = ?, de = reencodage(?), zh = ?, modificateur = ?, date_modification = current_date WHERE id = ?;" );
