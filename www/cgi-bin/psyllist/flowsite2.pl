...
if ($xbase eq 'psylles') {	
	...
	if ($xpage eq 'explorer') {
		...
		$content = 	div({-style=>'text-align: center;'},
						...
						p, br, p,
						div({-class=>'rotate30 tampon'}, "Psylles new version, v.8:", br, "new interface, new data", br, "and graphical display of name history" ),
						div({-style=>'font-size: 66px; width: 200px; margin-left: 400px;'}, 'PSYLLES'),
						p, br, p, 
...
my $icons = 	div({-class=>'info', -id=>"families", -style=>'position: absolute; z-index: 10; display: none;'}, ucfirst($traduction->{'families'}->{$xlang})).
		...
		a({-href=>$url."?db=$xbase&page=explorer&card=cavernicolous&lang=$xlang", -style=>'text-decoration: none;', -onMouseOver=>"makeBulle(this, 'cavernicolous');", -onMouseOut=>"document.getElementById('cavernicolous').style.display = 'none';"}, img({-src=>$docpath.'wheel/img/cavernicolous.png', -alt=>"Cavernicolous", -class=>'icon1'})). ' '.
