...
if (url_param('db')) { $xbase = url_param('db'); } else { $xbase = 'cool'; }
...
# traductions
my $config_file = '/etc/flow/coolexplorer.conf';
...
foreach my $key (sort {$types{$a} cmp $types{$b}} keys(%types)) {
	$search_fields .= textfield(	-name=>"search$z",
					...
					-style=>"z-index: $z;",
					-id=>"$key",
					-onFocus=>"if(this.value != '".ucfirst($traduction->{search}->{$xlang})."'){ AutoComplete_ShowDropdown(this.getAttribute('id')); } else { this.value = '' }",
					...
					div({-style=>'text-align: center;'},
						...
						table({-border=>0}, 
							Tr(	td({-style=>'width: 300px; text-align: center;'},$mnhn), 
								td({-style=>'width: 400px; text-align: center; font-size: 66px;', -rowspan=>2}, 'COOL'), 
...
# %0D%0A
if ($searchid) { 
	$msgbody .= " - ".url()."?db=flow;page=explorer;card=searching;searchid=$searchid;searchtable=noms_complets;lang=en";
}
else { 
...
#	my $sm20 = a({-href=>"http://hemiptera.infosyslab.fr/dbtnt/", -style=>'text-decoration: none;', -target=>"_blank"}, 'DBTNT');	
	...
	# Footer links update by vannicnonongo@gmail.com
...
sub search_formating {
	...
	foreach (@{$arr}) {
		...
		if ($table ne 'noms_complets') {
			if ($_->[1] =~ m|[^A-Z a-z 0-9 : , ( ) \[ \] ! _ = & Â° . * ; â€œ " â€™ â€� Ã©' \\ \/ \- â€“ ? â€¡ \n ]|) {
