...
elsif ($dbase eq 'aradides') { $config_file = '/etc/aradides.conf'; $synop_conf = '/etc/aradides.conf'; $pdfdir = 'aradpdf'; }
...
elsif ($dbase eq 'aleurodes') { $config_file = '/etc/aleurodsexplorer.conf'; $synop_conf = '/etc/aleurodseditor.conf'; }
elsif ($dbase eq 'tingides') { $config_file = '/etc/tingides.conf'; $synop_conf = '/etc/tingides.conf'; }
elsif ($dbase eq 'tessaratomidae') { $config_file = '/etc/tessaratomidae.conf'; $synop_conf = '/etc/tessaratomidae.conf'; }
elsif ($dbase eq 'lucanidae') { $config_file = '/etc/lucanidae.conf'; $synop_conf = '/etc/lucanidae.conf'; }
elsif ($dbase eq 'brentidae') { $config_file = '/etc/brentidae.conf'; $synop_conf = '/etc/brentidae.conf'; }
elsif ($dbase eq 'diptera') { $config_file = '/etc/diptera.conf'; $synop_conf = '/etc/diptera.conf'; }
elsif ($dbase eq 'test') { $config_file = '/etc/testexplorer.conf'; $synop_conf = '/etc/testeditor.conf'; }
elsif ($dbase eq 'hefo') { $config_file = '/etc/hefo.conf'; $synop_conf = '/etc/hefo.conf'; }
elsif ($dbase eq 'cipa') { $config_file = '/etc/cipaexplorer.conf'; }
elsif ($dbase eq 'dbtnt') { $config_file = '/etc/dbtntexplorer.conf'; }
...
sub taxon_card {
	...
	if ( my $dbc = db_connection($config) ) {
		...
		foreach (@{$leaves}) {
			my @phpcols;
			push(@phpcols, "$_->[3] ++ $_->[4] ++ https://hemiptera.infosyslab.fr/flow/?page=explorer&db=flow&lang=en&card=taxon&rank=$_->[1]&id=$_->[0]");
			my $parent = $_->[2];
			my $rank_pos = 1;
			while ($parent and $rank_pos < scalar(@{$sons_ranks})) {
				...
				if($ascendant->[0]) {
					...
					if($rank_pos < scalar(@{$sons_ranks})) {
						push(@phpcols, "$ascendant->[0][2] ++ $ascendant->[0][3] ++ https://hemiptera.infosyslab.fr/flow/?page=explorer&db=flow&lang=en&card=taxon&rank=$ascendant->[0][1]&id=$parent");
						$rank_pos++;
		...
		unless ($display_modes{synonymy}{skip} and $display_modes{chresonymy}{skip} and $mode ne 'full') {
			...
			if ( scalar @{$names_list} ) {
				...
				if (( !$display_modes{graphic}{skip} or $mode eq 'full') and !$toomuch) {
					...
					if ($dbase eq 'cool') {
						$graphic .= start_form(-id=>'graphForm', -method=>'POST', -action=>'https://hemiptera.infosyslab.fr/explorerdocs/graph.php', -target=>"graphFrame", -style=>'margin: 0; padding: 0;');
						...
						my $fullscreen;
						#if ($dbase eq 'flow') {
							$fullscreen .= start_form(-id=>'graphForm', -method=>'POST', -action=>'https://hemiptera.infosyslab.fr/explorerdocs/graph.php', -target=>"graphFrame", -style=>'margin: 0; padding: 0;');
