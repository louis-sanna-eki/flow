...
if ($dbase eq 'flows') { $config_file = '/etc/flow/flowexplorer.conf'; $synop_conf = '/etc/flow/floweditor.conf'; $pdfdir = 'flowpdf'; }
...
elsif ($dbase eq 'psylles2') { $config_file = '/etc/flow/psyllesexplorer2.conf'; $pdfdir = 'psyllespdf'; }
elsif ($dbase eq 'aradides') { $config_file = '/etc/flow/aradexplorer.conf'; $synop_conf = '/etc/flow/aradeditor.conf'; $pdfdir = 'aradpdf'; }
...
my %scripts = ( 
	...
	'psylles2'	=> '/cgi-bin/psyllesexplorer2.pl?',
	'aradides'	=> '/cgi-bin/aradexplorer.pl?',
	'coleorrhyncha'	=> '/cgi-bin/coleorrhyncha.pl?',
	'cerambycidae'	=> '/cgi-bin/cerambycidae.pl?',
	'oldflow'	=> '/cgi-bin/oldflowsite.pl?page=explorer&',
	'cipa'		=> '/cgi-bin/cipaexplorer.pl?'
);
...
sub topics_list {
	...
	if ($dbase eq 'psylles') {
		...
		foreach (@topics) {
			...
			$topics_list .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>$href}, $trans->{$label}->{$lang}) );
		}
		$topics_list .= end_ul();
...
sub families_list {
	if ( my $dbc = db_connection($config) ) { 
		...
		my $sth = $dbc->prepare( "SELECT count(*) FROM taxons AS t LEFT JOIN taxons_x_noms AS txn ON t.index = txn.ref_taxon
		...
		# Fetch families
		my ( $taxonid, $name, $autority );
		$sth = $dbc->prepare( "SELECT t.index, n.orthographe, n.autorite FROM taxons AS t LEFT JOIN taxons_x_noms AS txn ON t.index = txn.ref_taxon
								LEFT JOIN noms_complets AS n ON txn.ref_nom = n.index
								LEFT JOIN rangs AS r ON t.ref_rang = r.index
								LEFT JOIN statuts AS s ON txn.ref_statut = s.index
								WHERE r.en = 'family' AND s.en = 'valid'
								...
								$bornes;");
		$sth->execute( );
		$sth->bind_columns( \( $taxonid, $name, $autority ) );
		my $fa_list = start_ul({-class=>'exploul'});
		while ( $sth->fetch() ){
			$fa_list .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=family&id=$taxonid"}, i("$name") . " $autority" ) );
		}
		$fa_list .= end_ul();
		
		my $prevnext;
		unless ($dbase eq 'cool') { $prevnext = prev_next_topic($card); }
		...
		$fullhtml =  	div({-class=>'explocontent'},
					...
					$prevnext,
					div({-class=>'carddiv'},
						h2({-class=>'exploh2'}, $trans->{"FA_list"}->{$lang}),
						#h3({-class=>'exploh3'}, "$fa_numb $trans->{'fam(s)'}->{$lang}"),
						$fa_list
					)
				);
...
sub genera_list {
	if ( my $dbc = db_connection($config) ) { # connection
		...
		# Fetch genera
		my ( $taxonid, $name, $autority, $parent_name, $parent_taxon, $parent_rank);
		
		my $req = 	"SELECT t.index, n.orthographe, n.autorite, n2.orthographe, t.ref_taxon_parent,  r2.en FROM taxons AS t 
				LEFT JOIN taxons_x_noms AS txn ON t.index = txn.ref_taxon
				LEFT JOIN noms_complets AS n ON txn.ref_nom = n.index
				LEFT JOIN statuts AS s ON txn.ref_statut = s.index
				LEFT JOIN rangs AS r ON t.ref_rang = r.index
		...
		$sth->bind_columns( \( $taxonid, $name, $autority, $parent_name, $parent_taxon, $parent_rank ) );
		...
		$fullhtml =  	div({-class=>'explocontent'},
					...
					$prevnext,
					div({-class=>'carddiv'},
						h2({-class=>'exploh2'}, $trans->{"GE_list"}->{$lang}), # title
						$alphabet,
						#h3({-class=>'exploh3'}, "$ge_numb $trans->{'gen(s)'}->{$lang}"),
						$ge_list
					)
				);
	...
	if ( my $dbc = db_connection($config) ) {
		...
		$fullhtml =  	div({-class=>'explocontent'},
					...
					div({-class=>'carddiv'},
						...
						$alphabet,
						#h3({-class=>'exploh3'}, "$sp_numb $trans->{'species_l'}->{$lang}"),
						$sp_list
					)
				);
...
sub names_list {
	...
	my ($ordstr, $famstr, $genstr, $spestr) = ($trans->{'ord_key'}->{$lang}, $trans->{'families'}->{$lang}, $trans->{'gen_key'}->{$lang}, $trans->{'speciess'}->{$lang});
	...
	if ( my $dbc = db_connection($config) and $rank ) {
		...
		if ( $rank eq 'order' or $rank eq 'suborder' ){
			...
			foreach my $name ( @{$names} ){
				if ( $name->[3] eq 'valid' ){
					$fam .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=family&id=$name->[6]"}, i($name->[1]) . " $name->[2] &nbsp;" . b("$name->[4]") ) );				
				}
				else {
			...
			foreach my $name ( @{$names} ){	
				if ( $name->[3] eq 'valid' ){
                                        $gen .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=genus&id=$name->[6]"}, i($name->[1]) . " $name->[2] &nbsp;" . b("$name->[4]") ) );
				}
				else {
			...
			foreach my $name ( @{$names} ){
				if ( $name->[3] eq 'valido' ){
					$spec .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$name->[6]"}, i($name->[1]) . " $name->[2] &nbsp;" . b("$name->[4]")) );
				}
				else {
...
sub authors_list {
	if ( my $dbc = db_connection($config) ) {
		...
		if ($au_numb->[0] > 100) {
			...
			$au_numb = request_row("SELECT count(*) FROM auteurs WHERE nom ILIKE '$alph%';",$dbc);
			
			my $vlreq = "	SELECT lower(substring(nom,1,1)) AS letter, count(*) 
					FROM auteurs AS a
					...
					HAVING count(*) > 0 
					ORDER BY lower(substring(nom,1,1));";
		...
		$fullhtml =  	div({-class=>'explocontent'},
					...
					$prevnext,
					div({-class=>'carddiv'},
						h2({-class=>'exploh2'}, $trans->{"AU_list"}->{$lang}),
						$alphabet,
...
sub publications_list {
	if ( my $dbc = db_connection($config) ) {
		...
		my ($nbpubs) = @{ request_row('SELECT count(*) FROM publications', $dbc)};
		...
		my $dreq = 'SELECT nom, count(*) FROM auteurs_x_publications LEFT JOIN auteurs ON ref_auteur = index WHERE position = 1 GROUP BY nom ORDER BY nom;';
...
sub plants_list {
	if ( my $dbc = db_connection($config) ) {
		...
		my $plants = request_tab("SELECT p1.index, p1.autorite, p2.nom, p1.nom FROM plantes AS p1 
						LEFT JOIN plantes AS p2 ON (p1.ref_parent = p2.index)
						WHERE p1.nom ILIKE '$alph%'
						AND p1.ref_rang in (SELECT index FROM rangs WHERE en in ('genus'))
						AND p1.index in (SELECT distinct ref_plante FROM taxons_x_plantes)
						ORDER BY p1.nom, p2.nom
						$bornes;",$dbc);

		my $plants2 = request_tab("SELECT p1.index, p1.autorite, p3.nom, p2.nom, p1.nom FROM plantes AS p1 
						LEFT JOIN plantes AS p2 ON (p1.ref_parent = p2.index)
						LEFT JOIN plantes AS p3 ON (p2.ref_parent = p3.index)
						WHERE p1.nom ILIKE '$alph%'
						AND p1.ref_rang in (SELECT index FROM rangs WHERE en in ('species'))
						AND p1.index in (SELECT distinct ref_plante FROM taxons_x_plantes)
		...
		if (url_param('mode') eq 'family') {
			my @sorted = sort {$a->[2] cmp $b->[2] || $a->[3] cmp $b->[3]} @{$plants},@{$plants2};
			
			my $current;
			foreach my $row ( @sorted ){
				my $pdisp;
				if ($row->[4]) { $pdisp = i("$row->[3] $row->[4]") . " $row->[1]";}
				else { $pdisp = i("$row->[3]")." $row->[1]";}
				if (!$current) { $plants_list .= li({-class=>'exploli'}, br . div($row->[2])); $current = $row->[2]; }
				elsif ($row->[2] ne $current) { $plants_list .= li({-class=>'exploli'}, br . div($row->[2])); $current = $row->[2]; }
				$plants_list .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=plant&id=$row->[0]"}, $pdisp));
			}
			...
			my @sorted = sort {$a->[3] cmp $b->[3]} @{$plants},@{$plants2};
			
			$plants_list .= br;
			foreach my $row ( @sorted ){
				my $pdisp;
				if ($row->[4]) { $pdisp = i("$row->[3] $row->[4]") . " $row->[1]"; $row->[2] = " ($row->[2])"}
				else { $pdisp = i("$row->[3]")." $row->[1]"; $row->[2] = " ($row->[2])"}
				
				$plants_list .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=plant&id=$row->[0]"}, $pdisp).$row->[2]);
			}
		...
		$fullhtml =  	div({-class=>'explocontent'},
					...
					$prevnext,
					div({-class=>'carddiv'},
						h2({-class=>'exploh2'}, $trans->{"PL_list"}->{$lang}),
						#h3({-class=>'exploh3', -style=>'display: inline;'}, scalar(@{$plants}, @{$plants2}) . " $trans->{'ph_key'}->{$lang}") .
...
sub countries_list {
	if ( my $dbc = db_connection($config) ) {
		...
		$fullhtml =  	div({-class=>'explocontent'},
					...
					$prevnext,
					div({-class=>'carddiv'},
						h2({-class=>'exploh2'}, $trans->{"CN_list"}->{$lang}),
						$alphabet,
...
sub images_list {
		...
		if ( my $dbc = db_connection($config) ) {
			...
			my $req = "		SELECT i.index, i.icone_url, txn.ref_taxon, nc.index, nc.orthographe, nc.autorite, i.url
			...
			if (scalar(@{$images})) {
				...
				$html .= h2({-class=>'exploh2'}, $trans->{"type_img(s)"}->{$lang}) . p . 
				a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$images->[0][2]"}, i("$images->[0][4]") .  " $images->[0][5]") . p;
				my $current = $images->[0][3];
				foreach my $row ( @{$images} ){
						if ($row->[3] ne $current) {
							$html .= div({-style=>'clear: both;'}) . br . p . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$row->[2]"}, i("$row->[4]") .  " $row->[5]") . p;
							$current = $row->[3];
						}
			...
			$req = "		SELECT i.index, i.icone_url, txi.ref_taxon, nc.index, nc.orthographe, nc.autorite, i.url
			...
			if (scalar(@{$images})) {
				...
				$html .= h2({-class=>'exploh2'}, $trans->{"img(s)"}->{$lang}) . p . 
				a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$images->[0][2]"}, i("$images->[0][4]") .  " $images->[0][5]") . p;
				my $current = $images->[0][3];
				foreach my $row ( @{$images} ){
						if ($row->[3] ne $current) {
							$html .= 	div({-style=>'clear: both;'}) . br . p . 
											a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$row->[2]"}, i("$row->[4]") .  " $row->[5]"
										) . p;
							$current = $row->[3];
						}
						if ($dbase eq 'cool') {
...
sub vernaculars {
	if ( my $dbc = db_connection($config) ) { 
		...
		# Fetch families
		my ( $id, $name, $langg, $pays, $taxid, $tax, $aut, $rk, $ref_pays );
		...
		if ($mode eq 'country' or $mode eq 'language') {
			...
			if ($mode eq 'country') {
				...
				$order = 'l.langage, v.nom, p.en';
				$mode2 = 0;
			}
			
			$sth = $dbc->prepare( "SELECT distinct v.index, v.nom, l.langage, p.en, txn.ref_taxon, nc.orthographe, nc.autorite, r.en, v.ref_pays
						...
						ORDER BY $order;");
			$sth->execute( );
			$sth->bind_columns( \( $id, $name, $langg, $pays, $taxid, $tax, $aut, $rk, $ref_pays ) );

			my $current;
			while ( $sth->fetch() ){	
				if ($mode2) {
					if ($pays) {
						if (!$current) { $list .= li({-class=>'exploli'}, br . div($pays)); $current = $pays; }
						elsif ($pays ne $current) { $list .= li({-class=>'exploli'}, br . div($pays)); $current = $pays; }
					}
					else {
						if ($current ne 'others') { $list .= li({-class=>'exploli'}, br . div($trans->{'unassigned'}->{$lang})); $current = 'others'; }
					}
				}
				else {
					if (!$current) { $list .= li({-class=>'exploli'}, br . div($langg)); $current = $langg; }
					elsif ($langg ne $current) { $list .= li({-class=>'exploli'}, br . div($langg)); $current = $langg; }
				}
		...
		$fullhtml =  	div({-class=>'explocontent'},
					...
					div({-class=>'carddiv'},
						...
						#h3({-class=>'exploh3'}, "$numb $trans->{'fam(s)'}->{$lang}"),
...
sub repositories_list {
	if ( my $dbc = db_connection($config) ) {
		...
		foreach my $row ( @{$repositories} ){
			$de_tab .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=repository&id=$row->[0]"}, "$row->[1]," ) . " $row->[2]" );
		}
		$de_tab .= end_ul();
		...
		$fullhtml = 	div({-class=>'explocontent'},
					div({-class=>'carddiv'},
						h2({-class=>'exploh2'}, $trans->{"DE_list"}->{$lang}),
						h3({-class=>'exploh3'}, "$de_numb->[0] $trans->{'repositories'}->{$lang}"),
						$de_tab
					)
				);
...
sub eras_list {
	if ( my $dbc = db_connection($config) ) {
		...
		$fullhtml = 	div({-class=>'explocontent'},
					div({-class=>'carddiv'},
						h2({-class=>'exploh2'}, $trans->{"ER_list"}->{$lang}),
...
sub habitats_list {
	if ( my $dbc = db_connection($config) ) {
		...
		$fullhtml = 	div({-class=>'explocontent'},
					div({-class=>'carddiv'},
						h2({-class=>'exploh2'}, $trans->{"HA_list"}->{$lang}),
...
sub makeboard {
	...
	if ( my $dbc = db_connection($config) ) {
		...
		foreach my $suborder ( @{$suborders} ){
			...
			foreach my $superfamily ( @{$superfamilies}, @{$suborders} ){
...
sub board {
	if ( my $dbc = db_connection($config) ) {
		...
		if ($dbase ne 'cool' and $dbase ne 'cerambycidae') { 
			$ptit = td({-class=>'synopla'}, b($trans->{'plants'}->{$lang}));
		...
		foreach my $row (@{$rows}) {
			...
			my $prow;
			if ($dbase ne 'cool' and $dbase ne 'cerambycidae') { $prow = td({-class=>'synopla'}, $row->[8]) }
		...
		if ($dbase ne 'flow' and $dbase ne 'cerambycidae' ) {
...
sub family_card {
	if ( my $dbc = db_connection($config) ) { 
		...
		my $name = request_row("SELECT t.index, n.orthographe, n.autorite, n.ref_publication_princeps, ni.page_princeps FROM taxons AS t LEFT JOIN taxons_x_noms AS txn ON t.index = txn.ref_taxon
			LEFT JOIN noms_complets AS n ON txn.ref_nom = n.index
			LEFT JOIN noms AS ni ON n.index = ni.index
			LEFT JOIN rangs AS r ON t.ref_rang = r.index
			LEFT JOIN statuts AS s ON txn.ref_statut = s.index
			WHERE s.en = 'valid' AND ref_taxon = $family_id;",$dbc);
		$fam_name = i("$name->[1]") . " $name->[2]";
		$fa_name = "$name->[1] $name->[2]";
		...
		if ( $name->[3] ) {
			...
			$publication_tab .= div({-class=>'subject'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$name->[3]"}, "$pub") . getPDF($name->[3]));
		}
		else {
		...
		if ($dbase ne 'cool') {
			...
			$up .=  div({-class=>'tolist'}, makeup('families', $trans->{'fam_key'}->{$lang}));
		}
		...
		my $sp_ids  = [];
		my $all_sp_ids = [];
		if (url_param('mode') eq 'full' or $mode eq 'full') {
			$ge_ids = son_taxa($family_id, $ranks_ids->{"genus"}->{"index"}, $dbc);

			if ( scalar @{$ge_ids} ){
			
				my $gids = "(" . join(',', @{$ge_ids}) . ")";
				my ( $genusid, $genusname, $genusautority );
				$sth = $dbc->prepare( "SELECT t.index, n.orthographe, n.autorite FROM taxons AS t
							LEFT JOIN taxons_x_noms AS txn ON t.index = txn.ref_taxon
							LEFT JOIN noms_complets AS n ON txn.ref_nom = n.index
							LEFT JOIN rangs AS r ON t.ref_rang = r.index
							LEFT JOIN statuts AS s ON txn.ref_statut = s.index
							...
							ORDER BY n.orthographe;" );
				$sth->execute( );
				$sth->bind_columns( \( $genusid, $genusname, $genusautority ) );
				
				$fam_taxa .= start_ul({-class=>'exploul'});
				while ( $sth->fetch() ){

					$fam_taxa .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=genus&id=$genusid"}, i("$genusname") . " $genusautority"));

					my $sp_ids = son_taxa($genusid, $ranks_ids->{"species"}->{"index"}, $dbc);
					#$fam_taxa .= h3({-class=>'exploh3'}, scalar @{$sp_ids} . " $trans->{'species_l'}->{$lang} ");
			
					if ( scalar @{$sp_ids} ){
						@{$all_sp_ids} = (@{$all_sp_ids}, @{$sp_ids});
						my $ids = "(" . join(',', @{$sp_ids}) . ")";
						my ( $speciesid, $speciesname, $speciesautority );
						my $sth2 = $dbc->prepare( "SELECT t.index, n.orthographe, n.autorite FROM taxons AS t 
									LEFT JOIN taxons_x_noms AS txn ON t.index = txn.ref_taxon
									LEFT JOIN noms_complets AS n ON txn.ref_nom = n.index
									LEFT JOIN rangs AS r ON t.ref_rang = r.index
									LEFT JOIN statuts AS s ON txn.ref_statut = s.index
									...
									ORDER BY n.orthographe;" );
						$sth2->execute( );
						$sth2->bind_columns( \( $speciesid, $speciesname, $speciesautority ) );
					
						while ( $sth2->fetch() ){
							$fam_taxa .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$speciesid"}, '&nbsp;&nbsp;&nbsp;&nbsp;' . i("$speciesname") . " $speciesautority"));
							$sps++;
						}
						$sth2->finish();
					}
				...
				$fam_taxa = 	h3({-class=>'exploh3', -style=>'display: inline;'}, scalar @{$ge_ids} . " $trans->{'gen(s)'}->{$lang}").
						h3({-class=>'exploh3', -style=>'display: inline;'}, " $sps $trans->{'species_l'}->{$lang}"). p.
						$fam_taxa;
			}
			else {
			...
			if (scalar @{$ge_ids}) {
				...
				$fam_taxa .= h3({-class=>'exploh3'}, scalar @{$ge_ids} . " $trans->{'gen(s)'}->{$lang}");
			
				$fam_taxa .= start_ul({-class=>'exploul'});
				foreach (@{$ge_ids}) { 
					$fam_taxa .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=genus&id=$_->[0]"}, i($_->[1]) . " $_->[2]" ));	
				}
		...
		my $map;
		my @countries;
		if ($dbase eq 'flow' or url_param('mode') eq 'full' or $mode eq 'full') {
			my %tgdone;
			my @tdwg4;
			my $areas;
			if ( scalar @{$all_sp_ids} or $family_id ){
				
				foreach my $species_id (@{$all_sp_ids}, $family_id) { 
			...
			if ($mapok) {
				...
				function ImageMax(chemin) {
					...
					<img id='cmap' style='background: #$bgsea;' src='http://edit.africamuseum.be/edit_wp5/v1/areas.php?$areas&$styles&ms=$wdth' onMouseOver=\"this.style.cursor='pointer';\"  onclick=\"ImageMax('http://edit.africamuseum.be/edit_wp5/v1/areas.php?$areas&$styles&ms=1000');\">". br. br;
			}
		}
		...
		if ( scalar(@countries) ){
			...
			$countries_list = 	span({-id=>'ctrdisp',	-class=>'disparrow',
	...
	if ( my $dbc = db_connection($config) ) {
		...
		if ($dbase ne 'cool') {
			$up = 	div({-class=>'navup'}, 
					...
					makeup('genera', $trans->{'GE_list'}->{$lang}, lc(substr($ge_name, 0, 1)))
				);
		}
		...
		if ( scalar @{$sg_ids} ){
			...
			if (scalar(@{$sg_list})) { 
				...
				if (scalar(@{$sg_list}) == 1) { 
					...
					$sg_tab .= h4({-class=>'exploh4'}, scalar @{$sg_list} . " ". ucfirst($trans->{'subgenus_l'}->{$lang})); 
				}
		...
		if ( scalar @{$sp_ids} ){
			...
			if (scalar(@{$sp_list})) { 
				...
				$sp_tab .= h4({-class=>'exploh4'}, scalar @{$sp_list} . " $trans->{'species_l'}->{$lang}");
		...
		foreach my $syn ( @{$synss} ){
			...
			if ( $syn->[15] eq 'synonym' or $syn->[15] eq 'junior synonym' ){
				...
				if ($pub_den[1]) {
					...
					$sl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$sl .= getPDF($syn->[5]);
				}
				...
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$tl .= " $trans->{'trans_in'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$tl .= getPDF($syn->[5]);
				}
				...
				if ($pub_den[1]) {
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$mil .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$mil .= getPDF($syn->[5]);
				}
				...
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$pil .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$pil .= getPDF($syn->[5]);
				}
				...
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$iol .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$iol .= getPDF($syn->[5]);
				}
				...
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$iel .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$iel .= getPDF($syn->[5]);
				}
				...
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$hl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$hl .= getPDF($syn->[5]);
				}
				...
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$nl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$nl .= getPDF($syn->[5]);
				}
				...
				my $nl = a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=name&id=$syn->[0]"}, i($syn->[1]) . " $syn->[2]" );
				$nl .=  " " . i($syn->[3]);
			       	if ($syn->[1] ne $syn->[16] or $syn->[2] ne $syn->[17]) { $nl .= " $trans->{'en1'}->{$lang} " . i($syn->[16]) . " $syn->[17]"; }
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$nl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
				...
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$npl .= ", $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$npl .= getPDF($syn->[5]);
				}
				...
				if ($pub_den[1]) { 
					my $page;
					if ($syn->[19]) { $page = ": $syn->[19]" }
					$ukl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
					$ukl .= getPDF($syn->[5]);
				}
		...
		$fullhtml = 	div({-class=>'explocontent'},
					...
					div({-class=>'carddiv'},
						...
						h4({-class=>'exploh4'}, $trans->{'ori_pub'}->{$lang}), 
						$publication_tab,
						$display,
						$ch_tab,
						$sg_tab,
...
sub subgenus_card {
	if ( my $dbc = db_connection($config) ) {
		...
		if ($dbase ne 'cool') {
			$up = 	div({-class=>'navup'}, 
					$totop, span({-class=>'navarrow'},' > '),
					makeup('genera', $trans->{'GE_list'}->{$lang}, lc(substr($sge_name, 0, 1)))
				);
		}
...
sub species_card {
	...
	if ( my $dbc = db_connection($config) ) {
		...
		if ( scalar @{$ssp_ids} ){
			...
			if ( scalar(@{$ssp_list})) {
				if ( scalar(@{$ssp_list}) == 1 ){ 
					...
					$ssp_tab .= h4({-class=>'exploh4'}, scalar @{$ssp_list} . " $trans->{'subspecies_l'}->{$lang}"); 
				}
		...
		if ($dbase eq 'flow' or $dbase eq 'cipa') {
			$up .=  div({-class=>'navup'}, 
		...
		if (  $name->[2] ) {
			...
			$publication_tab = div({-class=>'subject'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$name->[2]"}, "$pub") . getPDF($name->[2]));
		}
		else {
			$publication_tab = div({-class=>'subject'}, $trans->{"UNK"}->{$lang});
		}
		...
		if ( scalar @{$names_list} != 0 or $display ) {
			...
			foreach my $syn ( @{$names_list} ){
				...
				if ( $syn->[3] eq 'synonym' or $syn->[3] eq 'junior synonym' ){
					...
					my $nl = a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=name&id=$syn->[0]"}, i($syn->[1]) . " $syn->[2]" );
					$nl .=  " " . i($syn->[17]);
					if ($syn->[1] ne $syn->[15] or $syn->[2] ne $syn->[16]) { $nl .= " $trans->{'en1'}->{$lang} " . i($syn->[15]) . " $syn->[16]"; }
					if ($pub_den[1]) { 
						my $page;
						if ($syn->[21]) { $page = ": $syn->[21]" }
						$nl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]$page" );
		...
		my $plants = request_tab("	SELECT 	p1.index, 
							p1.nom, 
							p2.nom, 
							p3.nom, 
		...
		my $tabpl;
		my $partial;
		if (($dbase eq 'psylles' or $dbase eq 'cool') and !$name->[8]) { $partial = " ($trans->{partial_distrib}->{$lang})"; } else { $partial = '' }
		if ( scalar @{$plants} ){
			foreach my $row ( @{$plants} ){
				...
				if ($row->[5] eq 'uncertain') { $confirm = "&nbsp; [ $trans->{'hpdbt'}->{$lang} ]"; }
			}

			if (scalar(@pubs)) { $string .= "&nbsp; $trans->{'segun'}->{$lang} &nbsp;" . join(', ', @pubs);  }
			
			if ($plants->[$#{$plants}][5] eq 'uncertain') { $confirm = "&nbsp; [ $trans->{'hpdbt'}->{$lang} ]"; }

			$tabpl .= li({-class=>'exploli'}, "$string $confirm");
			
			$tabpl = 	div({-id=>'pltdisp',	-class=>'disparrow',
								-style=>'display: block; float: left;',
		...
		my $sth5 = $dbc->prepare( "SELECT ref_pays, p.$lang, p.en, p.tdwg, p.tdwg_level, p.parent, ref_publication_ori, page_ori
						FROM taxons_x_pays AS txp 
						LEFT JOIN pays AS p ON txp.ref_pays = p.index
						LEFT JOIN publications  AS pub ON pub.index = ref_publication_ori
						WHERE txp.ref_taxon = $species_id and p.en != 'Unknown'
		...
		$sth5->execute( );
		my ( $country_id, $country, $en, $tdwg, $level, $parent, $ref_pub_ori, $page_ori );
		$sth5->bind_columns( \( $country_id, $country, $en, $tdwg, $level, $parent, $ref_pub_ori, $page_ori ) );
...
#				}
		...
		my $map;
		my $areas;
		if (($dbase eq 'psylles' or $dbase eq 'cool') and !$name->[7]) { $partial = " ($trans->{partial_distrib}->{$lang})"; } else { $partial = '' }
		if ( $countries_list ) {
			$countries_list = 	span({-id=>'ctrdisp',	-class=>'disparrow',
									-style=>'display: block; float: left;',
									-onMouseOver=>  "document.getElementById('ctrdisp').style.cursor = 'pointer';",
						...
						h4({-class=>'exploh4',	-id=>'cttitle',
									...
									-onClick=>"	if (document.getElementById('paysdiv').style.display == 'block') {
											...
											$trans->{"countries_as"}->{$lang}) . $partial . p .
		...
		my $speclinks;
		my $separator;
		if ($dbase eq 'flow') { $separator = hr({-class=>'spec_separator'}); }
...
sub subspecies_card {
	...
	if ( my $dbc = db_connection($config) ) {
		...
		my $sth2 = $dbc->prepare( "SELECT t.index, nc.orthographe, nc.autorite
								...
								LEFT JOIN statuts AS s ON txn.ref_statut = s.index
								LEFT JOIN rangs AS r ON t.ref_rang = r.index
								WHERE r.en = 'subspecies' AND s.en = 'valid'
								ORDER BY nc.orthographe;" );
		$sth2->execute( );
		$sth2->bind_columns( \( $current_id, $current_name, $current_authority ) );
		while ( $sth2->fetch() ){
		...
		if ( scalar @{$names_list} != 0 or $display ) {
			...
			foreach my $syn ( @{$names_list} ){
				...
				if ( $syn->[3] eq 'synonym' or $syn->[3] eq 'junior synonym' ){
					...
					$sl .= " $syn->[17] $trans->{'of'}->{$lang} " . i($syn->[15]) . " $syn->[16]";
					if ($pub_den[1]) { 
						$sl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$sl .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $sl) }
					elsif ($modal == 2) { $syn_list .= li({-class=>'exploli'}, $sl) }
					...
					$tl .= " $syn->[17] $trans->{'of'}->{$lang} " . i($syn->[15]) . " $syn->[16]";
					if ($pub_den[1]) { 
						$tl .= " $trans->{'trans_in'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$tl .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $tl) }
					elsif ($modal == 2) { $combi_list .= li({-class=>'exploli'}, $tl) }
					...
					if ($pub_den[1]) {
					       	$mil .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$mil .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $mil) }
					elsif ($modal == 2) { $wid_list .= li({-class=>'exploli'}, $mil) }
					...
					if ($pub_den[1]) { 
						$pil .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$pil .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $pil) }
					elsif ($modal == 2) { $pid_list .= li({-class=>'exploli'}, $pil) }
					...
					$iol .= " $syn->[17] $trans->{'of'}->{$lang} " . i($syn->[15]) . " $syn->[16]";
					if ($pub_den[1]) { 
						$iol .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$iol .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $iol) }
					elsif ($modal == 2) { $ios_list .= li({-class=>'exploli'}, $iol) }
					...
					$iel .= " $syn->[17] $trans->{'of'}->{$lang} " . i($syn->[15]) . " $syn->[16]";
					if ($pub_den[1]) { 
						$iel .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$iel .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $iel) }
					elsif ($modal == 2) { $ies_list .= li({-class=>'exploli'}, $iel) }
					...
					$hl .= " $syn->[17] $trans->{'of'}->{$lang} " . i($syn->[15]) . " $syn->[16]";
					if ($pub_den[1]) { 
						$hl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$hl .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $hl) }
					elsif ($modal == 2) { $hom_list .= li({-class=>'exploli'}, $hl) }
					...
					$nl .=  " " . i($syn->[17]) . " $trans->{'of'}->{$lang} " . i($syn->[15]) . " $syn->[16]";
					if ($pub_den[1]) { 
						$nl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$nl .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $nl) }
					elsif ($modal == 2) { $nn_list .= li({-class=>'exploli'}, $nl) }
					...
					$npl .= " " . i($syn->[17]) . " $trans->{'fromto'}->{$lang} " . i($syn->[15]) . " $syn->[16] " . i($trans->{'nnov'}->{$lang});
					if ($pub_den[1]) { 
						$npl .= ", $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$npl .= getPDF($syn->[5]);
					}
					if ($syn->[14] eq $name->[3] and 0) { $nomstat .= '&nbsp; ' . i($trans->{'nnov'}->{$lang}) }
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $npl) }
					...
					$ukl .= " $syn->[17] $trans->{'of'}->{$lang} " . i($syn->[15]) . " $syn->[16]";
					if ($pub_den[1]) { 
						$ukl .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$syn->[5]"}, "$pub_den[1]" );
						$ukl .= getPDF($syn->[5]);
					}
					if ($modal == 1) { $display .= li({-class=>'exploli'}, $ukl) }
					elsif ($modal == 2) { $uk_list .= li({-class=>'exploli'}, $ukl) }
...
sub name_card {
	if ( my $dbc = db_connection($config) ) {
		...
		my $name_id = $id;
		my $name = request_row("SELECT orthographe, autorite, ref_publication_princeps FROM noms_complets AS nc WHERE nc.index = $name_id;",$dbc);
		...
		my $taxa = request_tab("SELECT txn.ref_taxon, s.en, txn.ref_publication_utilisant, txn.ref_publication_denonciation, txn.exactitude,
					...
					txn.completude_femelle, txn.sexes_decrits, txn.sexes_valides, txn.ref_nom_cible, s.$lang, r.en, r2.en
					FROM taxons_x_noms AS txn 
					LEFT JOIN statuts AS s ON txn.ref_statut = s.index
					LEFT JOIN rangs AS r ON r.index = (select ref_rang from noms_complets where index = txn.ref_nom_cible)
					LEFT JOIN rangs AS r2 ON r2.index = (select ref_rang from noms_complets where index = txn.ref_nom)
					WHERE txn.ref_nom = $name_id
					AND s.en not in ('correct use');",$dbc);
		...
		if ($dbase ne 'cool') {
		 	$up = 	div({-class=>'navup'}, $totop );
			$up .=  div({-class=>'tolist'}, makeup('names', $trans->{'names'}->{$lang}, lc(substr($name->[0], 0, 1))));
		}
		...
		foreach my $taxon ( @{$taxa} ){
			...
			if ( $taxon->[1] eq 'synonym' or $taxon->[1] eq 'junior synonym'){
				...
				if ($pub_den[1]) { 
					$tt .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$taxon->[3]"}, "$pub_den[1]" );
					$tt .= getPDF($taxon->[3]);
				}
				...
				my $tt = "$taxon->[13] $trans->{'of'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=$taxon->[14]&id=$taxon->[0]"}, i($tax_name->[0]) . " $tax_name->[1]" );
				if ($pub_use[1]) { 
					$tt .= " $trans->{'dansin'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$taxon->[2]"}, "$pub_use[1]" ); 
					$tt .= getPDF($taxon->[2]);
				}
				if ($pub_den[1]) { 
					$tt .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$taxon->[3]"}, "$pub_den[1]" );
				...
				my $tt = "$taxon->[13] $trans->{'of'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=$taxon->[14]&id=$taxon->[0]"}, i($tax_name->[0]) . " $tax_name->[1]" );
				if ($pub_den[1]) { 
					$tt .= " $trans->{'trans_in'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$taxon->[3]"}, "$pub_den[1]" );
					$tt .= getPDF($taxon->[3]);
				}
				$taxa_tab .= li({-class=>'exploli'}, $tt);
			}
				...
				my $tt = i($taxon->[13]); 
				if ($tax_name->[1]) {
					$tt .= " $trans->{'en1'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=$taxon->[14]&id=$taxon->[0]"}, i($tax_name->[0]) . " $tax_name->[1]" );
				}
				if ($pub_den[1]) { 
					$tt .= " $trans->{'denounced'}->{$lang} " . a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$taxon->[3]"}, "$pub_den[1]" );
					$tt .= getPDF($taxon->[3]);
		...
		my $drvreq = "	SELECT index, orthographe, autorite FROM noms_complets AS nc
		...
		if (scalar @{$derives}) {
			...
			foreach ( @{$derives} ) {
				$drvnames .= li({-class=>'exploli'},  a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=name&id=$_->[0]"}, "$_->[1] $_->[2]"));
			}
			$drvnames .= end_ul();
		}
...
sub author_card {
	if ( my $dbc = db_connection($config) ) {
		...
		if ($dbase ne 'cool') {
		 	$up = 	div({-class=>'navup'}, $totop );
			$up .=  div({-class=>'tolist'}, makeup('authors', $trans->{'AU_list'}->{$lang}, lc(substr($current_surname, 0, 1))));
		}
		...
		if ( $sp_list->[0][0] ){
			...
			$sp_tab .= h4({-class=>'exploh4'}, "$size $subtitle");
			$sp_tab .= start_ul({-class=>'exploul'});
			foreach my $sp ( @{$sp_list} ){
				if ($current ne $sp->[6]) {
					my $sbt = $trans->{substr($current,0,3)."(s)"}->{$lang} || $curlang; 
					$sp_tab .= br. li({-class=>'exploli'}, "$count $sbt");
					$sp_tab .= $tmp_tab;
				...
				$tmp_tab .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$sp->[0]"}, i("$sp->[1]") . " $sp->[2]" ) );
				$count++;
			}
		        my $sbt = $trans->{substr($current,0,3)."(s)"}->{$lang} || $curlang;
			$sp_tab .= br. li({-class=>'exploli'}, "$count $sbt");        
		...
		if ( scalar @{$pub_list}){
			...
			#$pub_tab .= h3({-class=>'exploh3'}, $trans->{"pudescr"}->{$lang});
			$pub_tab .= h4({-class=>'exploh4'}, "$size $trans->{'publi_l'}->{$lang}");
...
sub publication_card {
	if ( my $dbc = db_connection($config) ) {
		...
		if ($dbase ne 'cool') {
		 	$up = 	div({-class=>'navup'}, $totop );
			$up .=  div({-class=>'tolist'}, makeup('publications', $trans->{'publications'}->{$lang}));
		}
...
#			}
		...
		while ( $sth->fetch ) {
			...
			$emend_list .= li({-class=>'exploli'}, 
						...
						" $trans->{'emended'}->{$lang} $trans->{'en1'}->{$lang} " . 
		...
		if ( $hp_list ){
			$hp_list = h4({-class=>'exploh4'}, ucfirst($trans->{'ph_key'}->{$lang})) . ul({-class=>'exploul'}, $hp_list);
		}		
		...
		$fullhtml = 	div({-class=>'explocontent'},
					...
					div({-class=>'carddiv'},
						...
						div({-class=>'subject', -id=>'pubtitle'}, $pub ), 
						$princeps_list,
						$syn_list,
						$transfer_list,
						$error_list,
...
sub plant_card {
	if ( my $dbc = db_connection($config) ) {
		...
		if ($dbase ne 'cool') {
		 	$up = 	div({-class=>'navup'}, $totop );
			$up .=  div({-class=>'tolist'}, makeup('plants', $trans->{'PL_list'}->{$lang}));
		}

		#my $up = prev_next_card( $card, $previous_id, $prev_name, $next_id, $next_name );

		my $sp_list = request_tab("SELECT t.index, n.orthographe, n.autorite, t.ref_taxon_parent, txp.ref_publication_ori, txp.certitude FROM taxons AS t LEFT JOIN taxons_x_plantes AS txp ON t.index = txp.ref_taxon
				LEFT JOIN taxons_x_noms AS txn ON t.index = txn.ref_taxon
				LEFT JOIN noms_complets AS n ON txn.ref_nom = n.index
				LEFT JOIN rangs AS r ON t.ref_rang = r.index
				LEFT JOIN statuts AS s ON txn.ref_statut = s.index
				WHERE r.en = 'species' AND s.en = 'valid' AND txp.ref_plante = $plant_id
				ORDER BY LOWER ( n.orthographe );",$dbc);

		my $sp_tab;
		if ( scalar @{$sp_list} != 0){
			my $size = scalar @{$sp_list};
			$sp_tab = h4({-class=>'exploh4'}, "$trans->{'hpof'}->{$lang} $size $trans->{'species_l'}->{$lang}");
			$sp_tab .= start_ul({-class=>'exploul'});
			foreach my $sp ( @{$sp_list} ){
				my $parent_name = [];
				$sp->[3] and $parent_name = request_row( "SELECT parent_taxon_name($sp->[3],'family');", $dbc );
				if ($sp->[4]) {
					my @pub = publication($sp->[4], 0, 1, $dbc);
					$sp->[4] = " &nbsp; $trans->{'segun'}->{$lang} &nbsp; " .
					a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=publication&id=$sp->[4]"}, $pub[1]) . getPDF($sp->[4]);
				}
				if ($dbase eq 'psylles') {
					if ($sp->[5] eq 'uncertain') { $sp->[5] = " [ $trans->{'hpdbt'}->{$lang} ]"}
					else { $sp->[5] = ''; }
				}
				else { $sp->[5] = ''; }
				
				$sp_tab .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$sp->[0]"}, i("$sp->[1]"), " $sp->[2] &nbsp;($parent_name->[0])"), $sp->[4], $sp->[5] );
			}
			$sp_tab .= end_ul();
		}
		else {
...
#		$sth2->finish();
		...
		my $up;
		if ($dbase eq 'cipa') { 
				$up = span({-class=>'navup'}, $totop ) . '&nbsp; > &nbsp;' . span({-class=>'tolist', -style=>'margin-left: 0;'}, makeup('countries', $trans->{'CN_list'}->{$lang}, lc(substr($country, 0, 1)))) . p;
		}
		elsif ($dbase ne 'cool') {
		 	$up = 	div({-class=>'navup'}, $totop );
			$up .=  div({-class=>'tolist'}, makeup('countries', $trans->{'CN_list'}->{$lang}, lc(substr($country, 0, 1))));
		}
		...
		my $sp_list = request_tab("SELECT txp.ref_taxon, n.orthographe, n.autorite
						...
						$croise
						group by txp.ref_taxon, n.orthographe, n.autorite
						ORDER BY LOWER ( n.orthographe );",$dbc);
		...
		if ( scalar @{$sp_list} != 0){
			...
			$sp_tab = h4({-class=>'exploh4'}, "$size $trans->{'species_l'}->{$lang}  $precise");
			$sp_tab .= start_ul({-class=>'exploul'});
			foreach my $sp ( @{$sp_list} ){
				$sp_tab .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$sp->[0]"}, i("$sp->[1]") . " $sp->[2]" ) . "$sp->[14]" );
			}
			$sp_tab .= end_ul();
		}
		else {
		...
		foreach (@{$regions}) {
			my $sp_list = request_tab("SELECT txp.ref_taxon, n.orthographe, n.autorite, p.$lang
							FROM taxons_x_pays AS txp LEFT JOIN taxons_x_noms AS txn ON txp.ref_taxon = txn.ref_taxon
							LEFT JOIN statuts AS s ON txn.ref_statut = s.index
							LEFT JOIN noms_complets AS n ON txn.ref_nom = n.index
			...
			if ( scalar @{$sp_list} != 0){
				my $size = scalar @{$sp_list};
				$sp_tab .= br . h4({-style=>'font-size: 17px;', -class=>'exploh4'}, $sp_list->[0][3]);
				$sp_tab .= h4({-class=>'exploh4'}, "$size $trans->{'species_l'}->{$lang} $precise");
				$sp_tab .= start_ul({-class=>'exploul'});
				foreach my $sp ( @{$sp_list} ){
					$sp_tab .= li({-class=>'exploli'}, a({-class=>'exploa', -href=>"$scripts{$dbase}db=$dbase&lang=$lang&card=species&id=$sp->[0]"}, i("$sp->[1]") . " $sp->[2]" ) . "$sp->[14]" );
		...
		if ( scalar @{$de_list} != 0){
			...
			$de_tab .= h4({-class=>'exploh4'}, "$size $trans->{'repos_l'}->{$lang}");
...
sub repository_card {
	if ( my $dbc = db_connection($config) ) {
		...
		#my $up = makeup('repositories', $trans->{'DE_list'}->{$lang});
		...
		$fullhtml = 	div({-class=>'explocontent'},
					div({-class=>'carddiv'},
						...
						div({-class=>'subject'}, "$repository->[0][0]. $repository->[0][1]"),
...
sub era_card {
	if ( my $dbc = db_connection($config) ) {
		...
		#my $up = makeup('eras', $trans->{'ER_list'}->{$lang});
		#$up .= prev_next_card( $card, $previous_id, $prev_name, $next_id, $next_name );
...
sub region_card {
	if ( my $dbc = db_connection($config) ) {
		...
		if ( scalar @{$sp_list} != 0){
			...
			$sp_tab .= h4({-class=>'exploh4'}, "$size $trans->{'species_l'}->{$lang}");
			$sp_tab .= start_ul({-class=>'exploul'});
			foreach my $sp ( @{$sp_list} ){
...
sub habitat_card {
	if ( my $dbc = db_connection($config) ) {
		...
		#my $up =  makeup('habitats', $trans->{'HA_list'}->{$lang});
		#$up .= prev_next_card( $card, $previous_id, $prev_name, $next_id, $next_name );
...
sub prev_next_card {
	...
	return div({-class=>'navnext0'}, '< ' . $prev_next_card . ' >');
}
...
sub son_fulltaxa {
	...
	my ( $index, $ref_rang, $ortho, $auth );
	my $sth = $dbh->prepare( "SELECT t.index, t.ref_rang, nc.orthographe, nc.autorite FROM taxons AS t 
					LEFT JOIN taxons_x_noms AS txn ON txn.ref_taxon = index
					...
					ORDER BY nc.orthographe;" );
	$sth->execute( );
	$sth->bind_columns( \( $index, $ref_rang, $ortho, $auth ) );
	while ( $sth->fetch() ){
		if ( $ref_rang == $rank ){
			push @{$list}, [$index, $ortho, $auth];
		}
		else {
...
sub completeness {
	...
	if ( $global eq '' or $male eq '' or $female eq '' ){
		if ( $global eq '' and $male eq '' and $female eq '' ){
			...
			$completeness = " " . $trans->{'uncomplete'}->{$lang};
		}
		elsif ( !$global and $male eq '' and !$female ){
			$completeness = " " . $trans->{'uncomplete'}->{$lang} . " " . $trans->{'amb_syn_F'}->{$lang};
		}
		elsif ( !$global and !$male and $female eq '' ){
			$completeness = " " . $trans->{'uncomplete'}->{$lang} . " " . $trans->{'amb_syn_M'}->{$lang};
		}
		else { # There must something wrong !
		}
	}
		...
		if ( $global and $male and $female ){
			...
			$completeness = " " . $trans->{'uncomplete'}->{$lang} . " " . $trans->{'amb_syn_M'}->{$lang} . " " . $trans->{'complete'}->{$lang} . " " . $trans->{'amb_syn_F'}->{$lang};
		}
		elsif ( !$global and $male and !$female ){
			$completeness = " " . $trans->{'complete'}->{$lang} . " " . $trans->{'amb_syn_M'}->{$lang} . " " . $trans->{'uncomplete'}->{$lang} . " " . $trans->{'amb_syn_F'}->{$lang};
		}
		else { # There must something wrong !
		}
	}
...
sub search_results {
	...
	if ($search) {
		...
		# Submit query string to the database
		if ( my $dbc = db_connection($config) ) {
			$query =~ s/\*/.*/g;
			$query =~ s/'/\\'/g;
			$query =~ s/\(/\\\\(/g;
			...
			my $req = 	"SELECT count(*)
					...
					LEFT JOIN statuts AS s ON txn.ref_statut = s.index
					LEFT JOIN rangs AS r ON t.ref_rang = r.index
					WHERE (r.en = 'family' OR r.en = 'genus' OR r.en = 'subgenus' OR r.en = 'species' OR r.en = 'subspecies')
					AND s.en not in ('correct use')
			...
			if ( $nbresults ){
				...
				my $sth2 = $dbc->prepare($req);					
				
				$sth2->execute( );
				my ( $taxonid, $name, $autority, $status, $rank, $nameid );
				$sth2->bind_columns( \( $taxonid, $name, $autority, $status, $rank, $nameid ) );
				
				if ( $nbresults > 1 ){
					$content .= div({-class=>'subject'},  "$nbresults $trans->{'match_names'}->{$lang}");
				...
				$content .= 	div({-class=>'subject'},  $trans->{'noresults'}->{$lang} . " : " . $query . " OR " . $query2);
		       }
