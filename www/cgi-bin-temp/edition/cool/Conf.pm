...
our $conf_file = '/etc/flow/cooleditor.conf';

our $imgs_url = 'http://hemiptera.infosyslab.fr/coolfotos/';

# Editor name
my $config = get_connection_params($conf_file);
my $dbh = db_connection($config, 'DEFAULT');
our $dblabel = $config->{'DB_NAME'} || 'COOL';
...
our $js = "
	...
	function testStatement(element) {
		...
		if (element.value != 0) { 
			...
			else if (document.getElementById('ref_pub3').value == '-- index --' || !document.getElementById('ref_pub3')) { 
			...
			else if (document.getElementById('ref_pub4').value == '-- index --' || !document.getElementById('ref_pub4')) { 
			...
			'taxons_x_documents' => {	
						...
						'definition' => [
									{	'type' 	=> 'foreign',
										...
										'ref' 	=> 'ref_document',
										'thesaurus' => 'documents',
										'addurl' => 'generique.pl?table=documents&new=1'
									}
								],
						'obligatory' => ['document', 'ref_document']
			},
