...
our $conf_file = '/etc/flow/diptera.conf';

# Editor name
my $config = get_connection_params($conf_file);
my $dbh = db_connection($config, 'EDITOR');
our $dblabel = $config->{'DB_NAME'} || 'DIPTERA';
...
my ($group) = @{request_row("SELECT orthographe FROM noms WHERE ref_rang = (SELECT index from rangs WHERE ordre = (SELECT min(ordre) FROM noms AS n LEFT JOIN rangs AS r ON r.index = n.ref_rang where ordre >= (SELECT ordre from rangs where en = 'order')));", $dbh)};
