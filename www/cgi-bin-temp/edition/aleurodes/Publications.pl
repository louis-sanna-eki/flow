#!/usr/bin/perl
BEGIN {push @INC, '/var/www/cgi-bin/edition/aleurodes/‘}
use strict;
...
my $dbc = db_connection(get_connection_params($conf_file),'EDITOR');
...
# C'est dans l'ordre get puis modify enfin update...
	...
	print redirection(param('pubType'), param('pubupdate'));

}

elsif (url_param('action') eq 'delete') {
	
	my $pubID = url_param('pubID');
	
	my $req = "BEGIN; DELETE FROM auteurs_x_publications WHERE ref_publication = $pubID; DELETE FROM publications WHERE index = $pubID; COMMIT;";
	
	my $sth = $dbc->prepare( $req ) or die $req.' : '.$dbc->errstr;
	
	#die $req;
		
	unless ($sth->execute()) { param('msg','Impossible to delete this publication, it is still used by the database'); }
	
	print redirection(param('pubType'), $pubID);
...
sub redirection {
	...
	my ($img, $msg);
	if (param('msg')) { 
		$img = img({-border=>0, -src=>'/Editor/stop.png', -name=>"stop"});
		$msg = span({-style=>"color: crimson;"}, param('msg')); 
		Delete('msg'); 
	}
	else { 
		$img = img({-border=>0, -src=>'/Editor/done.png', -name=>"done"});
		$msg = span({-style=>'color: green;'}, "Publication treated");
	}
	...
	if (!param('searchTo')) { 
		...
		if (url_param('action') ne 'delete') {
			
			my $param;
			if (!param('pubupdate')) { $param = 'get'; }
			else { $param = 'modify'; }
			
			$back = td({-style=>'padding-left: 10px;'}, img({-name=>'nameBack',
					-onMouseover=>"this.style.cursor='pointer'; document.getElementById('bulleN').innerHTML = 'Back';", 
					-onMouseOut=>"document.getElementById('bulleN').innerHTML = '';",
					-onClick=>"backform.action = 'Publications.pl?action=$param&page=pub'; backform.submit();", 
					-border=>0, 
					-src=>'/Editor/back.png'})),
				td({-id=>'bulleN', -style=>'width: 100px; color: darkgreen;'}, '');
		}
...
sub pub_form {
	...
	$titre = Tr(
			...
			td(textfield(-class=>'phantomTextField', -name=>'pubtitle', -style=>'width: 770px;', -onFocus=>AonFocus('pub')))
		);
	...
	$authors_field .= Tr(	td({-align=>'left',-valign=>'center'},span("Author $last ")),
				...
				div({-style=>'position: relative;'},
							...
							div( {-id=>'lessAuts', -onMouseOver=>"this.style.cursor = 'pointer';", -onClick=>"ChangeNbAuts(pubform, pubform.pubnbauts, 'less', $nbauts, 'Publications.pl', 'pub');", -style=>'position: absolute; top: 0px; left: 320px;'},img ({-src=>'/Editor/less.png', -border=>0}) ).
							div( {-id=>'moreAuts', -onMouseOver=>"this.style.cursor = 'pointer';", -onClick=>"ChangeNbAuts(pubform, pubform.pubnbauts, 'more', $nbauts, 'Publications.pl', 'pub');", -style=>'position: absolute; top: 0px; left: 345px;'},img ({-src=>'/Editor/more.png', -border=>0}) )
					)
				)
			);
	...
	if ($type eq 'Article') {
		...
		$revue = Tr(
				...
				td(
					table ({-border=>0, -cellspacing=>0, cellpadding=>0},Tr(
						td(popup_menu(-class=>'PopupStyle', -style=>'width: 800px;', -name=>"pubrevue",-values=>["",@revuesorder], -labels=>\%revueslabels, -onFocus=>AonFocus('pub'))),
		...
		if (scalar(@{$booklist})) {
			...
			my $bookselect = Tr( 
						...
						td(popup_menu(-class=>'PopupStyle', -name=>"bookSid", -values=>["",@sortedids], -labels=>\%pubs, -style=>'width: 800px;'))
					);
					#Tr(
	...
	if (url_param('action') eq 'modify') { $backaction = "	appendHidden(document.pubform, 'searchType', '$type');
								appendHidden(document.pubform, 'searchYear', ".param('pubyear').");
								appendHidden(document.pubform, 'searchFrom', 'dbtnt.pl');
								appendHidden(document.pubform, 'searchTo', 'Publications.pl?action=modify');
								appendHidden(document.pubform, 'NoNew', '1'); 
								document.pubform.action = 'pubsearch.pl?action=getOptions';
								document.pubform.submit();" 
	} 
		
	my @buttons;
	if ($backaction) {
		push(@buttons, td({-style=>'padding-left: 10px;'}, 
				img({	-name=>'pubBack',
	...
	if (my $pubID = param('pubupdate')) {
		push(@buttons, td({-style=>'padding-left: 10px;'}, 
				img({	-name=>'pubErase',
					-onMouseover=>"this.style.cursor='pointer'; document.getElementById('bulleD').innerHTML = 'Delete';", 
					-onMouseOut=>"document.getElementById('bulleD').innerHTML = '';",
					-onClick=>"if(confirm('Remove from the database?')) { document.pubform.action = 'Publications.pl?action=delete&page=pub&pubID=$pubID'; document.pubform.submit(); } else { return false; };", 
					-border=>0, 
					-src=>'/Editor/delete.png' })
			).	
			td({-id=>'bulleD', -style=>'width: 100px; color: darkgreen;'}, ''));
	}
	...
	$html .= html_header(\%headerHash).
		...
		div({-class=>'wcenter', -id=>'centre'},
			...
			"<FIELDSET class='wcenter round fieldset1'>",
			"<LEGEND class='round'>".span({-style=>'font-size: 18px; font-style: italic;'},"$type reference")."</legend>",
			...
			table({-cellspacing=>10, -id=>'mytable'},
				...
				Tr(td(),td(
					table({-border=>0},
						Tr(
							td(img({-style=>'margin-left: 0px;', 
								-onMouseover=>"this.style.cursor='pointer'; document.getElementById('bulleP').innerHTML = 'Submit';", 
								-onMouseOut=>"document.getElementById('bulleP').innerHTML = '';",
								-onClick=>"$form.action='Publications.pl?action=verify'; $form.submit();", 
								-border=>0, 
								-src=>'/Editor/ok.png', 
								-name=>'pubOk' })),
							td({-id=>"bulleP", -style=>'width: 100px; color: darkgreen;'}, ''),
							@buttons
						)
					)
				))
			...
			"<script type='text/javascript'>
				if (document.getElementById('mytable').offsetWidth > 900) { 
					document.getElementById('maintable').style.width = '900px'; 
					document.getElementById('centre').style.width = '900px'; 
				}
...
sub recapitulation {
	...
	$html .= html_header(\%headerHash).
		...
		div({-class=>'wcenter'},
			...
			table({-border=>0 },Tr(td({-style=>''}, $publi))), p,
