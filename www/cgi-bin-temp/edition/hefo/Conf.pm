...
our $conf_file = '/etc/flow/hefo.conf';
...
our $imgs_url = 'http://hemiptera.infosyslab.fr/flowfotos/';
...
my $periodlvls;
make_hash ($dbh, "SELECT index, en FROM niveaux_geologiques;", \$periodlvls);
my $lithos;
make_hash ($dbh, "SELECT index, en FROM lithostrats;", \$lithos);
my $litholvls;
make_hash ($dbh, "SELECT index, en FROM niveaux_litho;", \$litholvls);
my $typeTypes;
		...
		'taxons_x_sites' => {	
							...
							'definition' => [
									{	'type' 	=> 'foreign',
										...
										'id' 	=> 'localite',
										'ref' 	=> 'ref_localite',
										'thesaurus' => 'localities',
										'addurl' => 'generique.pl?table=localites&new=1'
									}, {
										'type' 	=> 'select',
										'title' => 'Lithostratigraphy',
										'id' 	=> 'ref_lithostrat',
										'values' => ['-- Lithostratigraphy --', sort { $lithos->{$a} cmp $lithos->{$b} } keys(%{$lithos})],
										'labels' => $lithos,										
										'addurl' => 'generique.pl?table=lithostrats&new=1'
									}, {
										'type' 	=> 'select',
										'title' => 'Chronostratigraphy start',
										'id' 	=> 'ref_periode',
										'values' => ['-- Chronostratigraphy start --', sort { $periods->{$a} cmp $periods->{$b} } keys(%{$periods})],
										'labels' => $periods,
										'addurl' => 'generique.pl?table=periodes&new=1'
									}, {
										'type' 	=> 'select',
										'title' => 'Chronostratigraphy end',
										'id' 	=> 'ref_periode2',
										'values' => ['-- Chronostratigraphy end --', sort { $periods->{$a} cmp $periods->{$b} } keys(%{$periods})],
										'labels' => $periods,
										'addurl' => 'generique.pl?table=periodes&new=1'
									}, {
										'type' 	=> 'pub',
										...
										'id' 	=> 'afficher',
										'values' => ['display','hide'],
									    'labels' => {'display'=>'display', 'hide'=>'hide'}
									}
								],
...
our $single_tables = {
	...
	'periodes' => {
		...
		'definition' => [	
					{	'type' => 'internal',
						...
						'title' => 'Level',
						'id' 	=> 'niveau',
						'values' => ['-- Level --', sort { $periodlvls->{$a} cmp $periodlvls->{$b} } keys(%{$periodlvls})],
						'labels' => $periodlvls,
		...
		'obligatory' => {'en'=>1, 'fr'=>1, 'es'=>1, 'de'=>1, 'pt'=>1, 'pl'=>1, level=>1}
	},
	'lithostrats' => {
		'title' => 'Lithostratigraphy',
		'definition' => [	
					{	'type' => 'internal',
						...
						'title' => 'Level',
						'id' 	=> 'niveau',
						'values' => ['-- Level --', sort { $litholvls->{$a} cmp $litholvls->{$b} } keys(%{$litholvls})],
						'labels' => $litholvls,
						'addurl' => 'generique.pl?table=niveaux_litho&new=1'
					}, {	
						'type' 	=> 'foreign',
						'title' => 'Parent',
						'id' 	=> 'higher',
	...
	'niveaux_geologiques' => {
		...
		'obligatory' => {'en'=>1, 'fr'=>1, 'es'=>1, 'de'=>1, 'pt'=>1, 'pl'=>1}
	},
	'niveaux_litho' => {
		'title' => 'Lithostratigraphic levels',
		'definition' => [	
					{	'type' => 'internal',
						'title' => 'English',
						'id'   => 'en',
...
sub get_single_table_parameters { 
	...
	elsif ($table eq 'lithostrats') { 
		${$title} = $single_tables->{$table}{'title'};
		${$singular} = 'Lithostratigraphy';
		${$label} = "x.en || coalesce(' [' || l.en || ']', '')";
		${$join} = 'LEFT JOIN niveaux_litho AS l ON l.index = x.niveau';
		${$order} = 'ORDER BY x.niveau';	
		${$champs} = $single_tables->{$table};
	}
	...
	elsif ($table eq 'niveaux_litho') { 
		${$title} = $single_tables->{$table}{'title'};
		${$singular} = 'Lithostratigraphic level';
		${$label} = 'x.en';
		${$order} = 'ORDER BY x.en';	
		${$champs} = $single_tables->{$table};
	}
...
sub get_single_table_thesaurus {
	...
	if ($table eq 'editions') { 
		...
		make_thesaurus("SELECT l.index, l.en FROM lithostrats AS l ORDER BY l.en;", 'lithos', '$intitule = $row->[1];', $retro, $onload, $dbc);
	}
}
