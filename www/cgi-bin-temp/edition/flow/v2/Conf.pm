	...
	'periodes' => {
		...
		'definition' => [	
					{	'type' => 'internal',
						...
						'id' 	=> 'level',
						'ref' 	=> 'niveau',
						'thesaurus' => 'niveaux_geologiques',
						'addurl' => 'generique.pl?table=niveaux_geologiques',
						'length' => 80,
						'message' => '<span style="color: navy; font-size: 20px;"> **</span>'
					}, {	
						'type' 	=> 'foreign',
...
sub get_single_table_thesaurus {
	...
	if ($table eq 'editions') { 
		...
		make_thesaurus("SELECT index, en, tdwg, tdwg_level FROM pays WHERE (tdwg_level in ('1', '2', '4') OR index in (SELECT index FROM pays WHERE tdwg_level = '3' and en not in (SELECT DISTINCT en FROM pays WHERE tdwg_level = '4')));", 'pays', '$intitule = "$row->[1] [$row->[3]]";', $retro, $onload, $dbc);
	}
	elsif ($table eq 'localites') { 
		...
		make_thesaurus("SELECT x.index, coalesce((get_host_plant(x.index)).nom,'') || coalesce(' ' || (get_host_plant(x.index)).autorite,'') FROM plantes AS x ORDER BY (get_host_plant(x.index)).nom;", 'plantes', '$intitule = $row->[1];', $retro, $onload, $dbc);
	}	
	elsif ($table eq 'periodes') { 
		make_thesaurus("SELECT p.index, p.en FROM periodes AS p ORDER BY p.en;", 'periodes', '$intitule = $row->[1];', $retro, $onload, $dbc);
		make_thesaurus("SELECT n.index, n.en FROM niveaux_geologiques AS n ORDER BY n.en;", 'niveaux_geologiques', '$intitule = $row->[1];', $retro, $onload, $dbc);
	}
}
