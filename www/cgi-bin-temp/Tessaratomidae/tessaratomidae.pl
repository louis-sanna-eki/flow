...
my $config_file = '/etc/flow/tessaratomidae.conf';
...
sub html_maker {
	...
	if ($webpage eq 'contributors') {
		$content = div({-class=>'infos'}, $trans->{contributors_tessaratomidae}->{$lang} || 'Soon available');
	}
	elsif ($webpage eq 'howtocite') {
		$content = div({-class=>'infos'}, 
			"Magnien, Ph. (<script type='text/javascript'>var currentTime = new Date();document.write(currentTime.getFullYear());</script>) - 
			$config->{DB_NAME} - http://www.hemiptera-databases.com/tessaratomidae - searched on 
		...
		$content = div({-class=>'infos'}, $trans->{recent_additions_tessaratomidae}->{$lang} || 'Soon available');
	}	
	...
	print	p;
	print	a({-href=>url()."?lang=$lang", -style=>"margin-left: 30px;"}, img({-src=>"/explorerdocs/logo_tessara_40.png", -height=>'40px', -style=>'border: 0;'})) . a({-href=>url()."?lang=$lang"}, span({-class=>'sitetitle'}, $config->{"DB_NAME"}));
	print	"<TABLE STYLE='margin-left: 35px; border: solid 0px #CCCCCC; width: 950px; text-align: center;' CELLPADDING=0 CELLSPACING=0>";
	print	"<TR><TD STYLE='text-align: left;;'>$trans->{BY}->{$lang} $config->{DB_AUTHORS}</TD><TD valign=center STYLE='background: transparent; border: 1px solid lightgrey; -moz-border-radius: 6px; -webkit-border-radius: 6px; border-radius: 6px;'>" . join('</TD><TD style="background: transparent; vertical-align: middle; text-align: center; padding: 2px;"></TD><TD style="border: 1px solid lightgrey; -moz-border-radius: 6px; -webkit-border-radius: 6px; border-radius: 6px; background: transparent; vertical-align: middle; text-align: center; padding: 4px 2px;">', @menus) . "&nbsp;</TD></TR>";
	print	"</TABLE>";
	print	p;
	...
	if ($content) {
		...
		print "<DIV class='contentContainer'>";
		print div({-style=>'margin: 20px 0;'}, $trans->{intro_tessaratomidae}->{$lang});
		print "</DIV>";
	}
