...
my $config_file = '/etc/flow/lucanidae.conf';
...
sub html_maker {
	...
	my $phts;
	foreach (keys(%photos)) { $phts .= '<li><img alt="" src="/lucafotos/'.$_.'.jpg"/></li>'; }
	...
	print	p;
	print	"<table><tr><td>".a({-href=>url()."?lang=$lang"}, span({-class=>'sitetitle'}, $config->{"DB_NAME"}))."</td><td style='padding-left: 480px;'>".a({-href=>"http://www.msn.unifi.it", -target=>'_blank'}, img({src=>"/brentfotos/museo.png", -height=>'60px', -style=>'border: 0;'}))."</td></tr></table>";
	print	"<TABLE STYLE='margin-left: 35px; border: solid 0px #CCCCCC; width: 950px; text-align: center;' CELLPADDING=0 CELLSPACING=0>";
	print	"<TR><TD STYLE='text-align: left;;'>$trans->{BY}->{$lang} $config->{DB_AUTHORS}</TD>";
	#print	"<TD valign=center STYLE='background: transparent; border: 1px solid lightgrey; -moz-border-radius: 6px; -webkit-border-radius: 6px; border-radius: 6px;'>" . join('</TD><TD style="background: transparent; vertical-align: middle; text-align: center; padding: 2px;"></TD><TD style="border: 1px solid lightgrey; -moz-border-radius: 6px; -webkit-border-radius: 6px; border-radius: 6px; background: transparent; vertical-align: middle; text-align: center; padding: 4px 2px;">', @menus) . "&nbsp;</TD>";
	print	"</TR>";
	print	"</TABLE>";
	print	p;
