...
my $config_file = '/etc/flow/diptera.conf';
...
if(scalar(@{$names})) { search_formating('noms_complets', $names, \$thesauri, \$onload, $dbc); }
if(scalar(@{$authors})) { search_formating('auteurs', $authors, \$thesauri, \$onload, $dbc); }
...
sub html_maker {
	...
	print	p;
	print	"<table><tr><td>".a({-href=>url()."?lang=$lang"}, span({-class=>'sitetitle'}, $config->{"DB_NAME"}))."</td><td style='padding-left: 450px;'>INSTITUTION LOGO</td></tr></table>";
